...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        await checkAuth();
        toast.success(`Welcome to Pushya Planet, ${name.split(" ")[0]}! 🎉`);
        router.push("/home");
      } else {
      ...
      <FadeIn className="w-full max-w-md">
        <Card className="border-gold/20 shadow-xl overflow-hidden">
          <div className="bg-gradient-to-b from-forest/5 to-transparent flex flex-col items-center pt-8 pb-4">
            ...
            />
            <h1 className="font-serif text-2xl font-bold text-forest">Create Account</h1>
            <p className="text-sm text-muted-foreground mt-1">Join the Pushya family today</p>
          </div>
            ...
            <form onSubmit={handleRegister} className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-forest/40" />
                  <input
                    type="text"
                    value={name}
                    onChange={(e) => setName(e.target.value)}
                    placeholder="Your full name"
                    className="pl-9 w-full py-2.5 rounded-md border border-forest/20 focus:border-gold focus:ring-1 focus:ring-gold text-sm"
                    required
                  />
                </div>
              </div>
                ...
                <div className="flex">
                  ...
                  <div className="relative flex-1">
                    ...
                    <input
                      ...
                      className="pl-9 w-full py-2.5 rounded-none rounded-r-md border border-forest/20 focus:border-gold focus:ring-1 focus:ring-gold text-sm"
                      inputMode="numeric"
                      required
                    />
                  </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="pl-9 pr-10 w-full py-2.5 rounded-md border border-forest/20 focus:border-gold focus:ring-1 focus:ring-gold text-sm"
                    required
                  />
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-forest/40">
              ...
              {/* Confirm Password */}
              <div className="space-y-1.5">
                <label className="text-sm font-medium text-forest">Confirm Password</label>
                <div className="relative">
                  <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-forest/40" />
                  <input
                    type={showPassword ? "text" : "password"}
              ...
              <p className="text-sm text-center text-muted-foreground">
                ...
                <Link href="/login" className="text-forest font-semibold hover:text-gold transition-colors">
                  Login
                </Link>
              </p>
            </form>
